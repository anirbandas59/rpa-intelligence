...
export default function Stage3Page() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (ucData.s3_inputs?.start_date)
          setStartDate(ucData.s3_inputs.start_date as string);
        if (ucData.s3_inputs?.complexity_class)
          setComplexityClass(
            ucData.s3_inputs.complexity_class as ComplexityClass,
          );
        if (ucData.s3_inputs?.phase_deltas)
          setPhaseDeltas(
            ucData.s3_inputs.phase_deltas as Record<string, number>,
          );

        if (ucData.s3_latest_run_id) {
          const latestRun = runsData.find(
            (r) => r.id === ucData.s3_latest_run_id,
          );
          if (latestRun?.status === "complete") {
            // List endpoint returns summary only — fetch full result from single-run endpoint
            const fullRun = await apiGet<{ result: S3Result }>(
              `/api/v1/use-cases/${ucId}/s3/runs/${ucData.s3_latest_run_id}`,
            );
  ...
  useEffect(() => {
    const taskExtractionStatus =
      typeof readiness?.s3 === "object"
        ? (readiness.s3 as S3ReadinessDetail).task_extraction
        : undefined;

    const isExtracting =
      taskExtractionStatus === "pending" ||
      taskExtractionStatus === "synthesizing";

    if (!isExtracting) return;

    const pollInterval = setInterval(async () => {
      try {
        const readinessData = await apiGet<ReadinessResponse>(
          `/api/v1/use-cases/${ucId}/readiness`,
        );
        setReadiness(readinessData);

        const status =
          typeof readinessData?.s3 === "object"
            ? (readinessData.s3 as S3ReadinessDetail).task_extraction
            : undefined;
  ...
  const handleLoadFromS2 = async () => {
    try {
      const result = await apiPost<S3LoadedResponse>(
        `/api/v1/use-cases/${ucId}/s3/load-from-s2`,
        {},
      );
      const { effort_weeks, complexity_class } = result.loaded_values;
      setEffortWeeks(effort_weeks);
      setComplexityClass(complexity_class);
      toast.success("Loaded from Stage 2");
    } catch (err) {
  ...
  const handleRunStage = async () => {
    ...
    try {
      ...
      // 3. Refetch all data (phase calculation completes immediately)
  ...
  const handlePhaseDelta = async (phaseName: string, delta: number) => {
    const newDeltas = {
      ...phaseDeltas,
      [phaseName]: (phaseDeltas[phaseName] || 0) + delta,
    };
    setPhaseDeltas(newDeltas);
    try {
      await apiPatch(`/api/v1/use-cases/${ucId}/s3/phase-delta`, {
        phase_name: phaseName.toLowerCase(),
        delta_weeks: delta,
      });
    } catch (err) {
  ...
  const handleGenerateNarrative = async () => {
    ...
    try {
      await apiPost(`/api/v1/use-cases/${ucId}/s3/generate-narrative`, {});

      // Poll for narrative completion
      const pollNarrative = setInterval(async () => {
        if (!useCase?.s3_latest_run_id) return;

        const fullRun = await apiGet<{ result: S3Result }>(
          `/api/v1/use-cases/${ucId}/s3/runs/${useCase.s3_latest_run_id}`,
        );
        if (fullRun.result.narrative) {
          setLatestResult(fullRun.result);
          setGeneratingNarrative(false);
          clearInterval(pollNarrative);
  ...
  const extractionComplete =
    taskExtractionStatus === "complete" ||
    taskExtractionStatus === "synthesized";
  const extractionFailed = taskExtractionStatus === "failed";
      ...
      <StageHeader
        ...
        actions={
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            {isExtracting && (
              <Pill color="var(--c-violet)">
                <Loader2 className="h-3 w-3 animate-spin" /> Extracting tasks...
              </Pill>
            )}
            {extractionComplete && (
              <Pill color="var(--c-green)">
                <Icon name="check" size={11} /> Extraction complete
              </Pill>
            )}
            {extractionFailed && (
              <Pill color="var(--c-red)">
                <Icon name="alert" size={11} /> Extraction failed
              </Pill>
            )}
          ...
          >
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={(v) => setComplexityClass(v as ComplexityClass)}
              >
                <SelectTrigger className="w-full h-10">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  {(["XS", "S", "M", "L", "XL"] as ComplexityClass[]).map(
                    (c) => (
        ...
        {latestResult && (
          <>
              ...
              <div style={{ display: "flex", gap: 14 }}>
                {(
                  [
                    {
                      label: "Start",
                      value: fmtDate(firstPhase.start_date),
                      icon: "calendar" as const,
                      color: "var(--c-blue)",
                    },
                    {
                      label: "Build effort",
                      value: `${buildPhase ? buildPhase.weeks + (phaseDeltas[buildPhase.name] || 0) : "—"} wks`,
                      icon: "clock" as const,
                      color: "var(--primary)",
                    },
                    {
                    ...
                    {
                      label: "Go-live",
                      value: fmtDate(lastPhase.end_date),
                      icon: "check" as const,
                      color: "var(--c-green)",
                    },
                  ] as const
            ...
            >
              ...
              >
                <SectionLabel>
                  Delivery Gantt · adjust phases with +/- buttons
                </SectionLabel>
                <div
                  style={{
                    display: "flex",
              ...
              {/* Week axis */}
              <div
                style={{ display: "flex", paddingLeft: 168, marginBottom: 6 }}
              >
                  ...
                  {ganttPhases.map((phase, idx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {/* Gantt track */}
                        <div
                          style={{ flex: 1, position: "relative", height: 26 }}
                        >
                          <div
                            style={{
                              position: "absolute",
                              ...
                              padding: "0 8px",
                              border: delta
                                ? "1.5px dashed color-mix(in oklab, var(--c-amber) 80%, white)"
                                : "none",
                            }}
                          >
                            <button
            ...
            {(latestResult.narrative || generatingNarrative) && (
              <Card
                style={{
                  ...
                  gap: 12,
                  background:
                    "linear-gradient(160deg, color-mix(in oklab, var(--c-violet) 8%, var(--surface)), var(--surface) 70%)",
                }}
              >
                <div
                  style={{
                  ...
                  >
                    <span
                      style={{
                        ...
                        justifyContent: "center",
                        background:
                          "color-mix(in oklab, var(--c-violet) 18%, transparent)",
                        color: "var(--c-violet)",
                      }}
                    >
                  ...
                  <div style={{ display: "flex", gap: 8 }}>
                    {!latestResult.narrative && (
                      <Btn
                        ...
                        icon={generatingNarrative ? undefined : "spark"}
                      >
                        {generatingNarrative && (
                ...
                {generatingNarrative && !latestResult.narrative && (
                  <div
                    style={{
                      display: "flex",
                      alignItems: "center",
                  ...
                  >
                    ...
                    <span style={{ fontSize: 12, color: "var(--muted-fg)" }}>
                      AI is crafting your executive summary...
                    </span>
                  </div>
                )}
                {latestResult.narrative && (
                  ...
                  >
                    <ReactMarkdown>{latestResult.narrative}</ReactMarkdown>
                  </div>
                )}
              </Card>
            )}

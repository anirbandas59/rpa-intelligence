...
import { ArrowLeft, Loader2, Play, RefreshCw, BarChart3, LayoutList } from "lucide-react"
import { toast } from "sonner"
import { apiGet, apiGetRuns, apiPost, apiPatch, isAuthenticated } from "@/lib/api"
import type {
  UseCase, StageRun, S1Result, ReadinessResponse, MigrationDecision, Confidence,
} from "@/lib/types"
...
const S1_DIMS = [
  { key: "technical_feasibility" as const, label: "Technical feasibility", max: 40, color: "var(--c-blue)" },
  { key: "migration_effort"      as const, label: "Migration effort",       max: 25, color: "var(--c-teal)" },
  { key: "platform_suitability"  as const, label: "Platform suitability",   max: 20, color: "var(--primary)" },
  { key: "risk"                  as const, label: "Risk (inverse)",          max: 15, color: "var(--c-amber)" },
]
...
export default function Stage1Page() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const [ucData, readinessData, runsData] = await Promise.all([
        ...
        apiGetRuns<StageRun>(`/api/v1/stage1/${ucId}/s1/runs`).catch(() => [] as StageRun[]),
      ])
      setUseCase(ucData)
      ...
      if (ucData.s1_latest_run_id && runsData.length > 0) {
        const latestRun = runsData.find((r) => r.id === ucData.s1_latest_run_id)
  ...
  const loadPortfolio = useCallback(async () => {
    ...
    try {
      const allUcs = await apiGet<UseCase[]>(`/api/v1/projects/${projectId}/use-cases`)
      const rows: PortfolioRow[] = await Promise.all(
        allUcs.map(async (u) => {
          if (!u.s1_latest_run_id) return { uc: u, result: null, status: "not_ready" }
          try {
            const ucRuns = await apiGetRuns<StageRun>(`/api/v1/stage1/${u.id}/s1/runs`)
            const latest = ucRuns.find((r) => r.id === u.s1_latest_run_id)
            if (latest?.status === "complete") {
              return { uc: u, result: latest.result as unknown as S1Result, status: "complete" }
            }
            return { uc: u, result: null, status: latest?.status || "not_ready" }
          } catch {
  ...
  const handleBackfill = async () => {
    try {
      ...
      toast.success("Backfill complete — re-run assessment to see updated inputs")
    } catch (err) {
  ...
  const handleScoreAll = async () => {
    ...
    if (unscored.length === 0) { toast.info("All use cases already scored"); return }
    setScoringAll(true)
    try {
      await Promise.all(unscored.map((r) => apiPost(`/api/v1/use-cases/${r.uc.id}/s1/runs`, {})))
        ...
        <div className="container flex h-14 items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">Stage 1 — Migration Assessment</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-1 rounded-lg border border-border/50 p-1">
              ...
              >
                <LayoutList className="h-3.5 w-3.5" />Portfolio
              </button>
            </div>
            ...
            <RunHistoryDrawer runs={runs} stage="s1" stageName="Stage 1 - Assessment" />
      ...
      <div className="container py-6 space-y-5">
          ...
          <>
            ...
            {!isComplete && !isRunning && (
              <div style={{
                ...
                <div>
                  <div style={{ fontSize: 17, fontWeight: 700, marginBottom: 6 }}>Run Migration Assessment</div>
                  <div style={{ fontSize: 13, color: "var(--muted-foreground)", maxWidth: 420 }}>
                    Haiku evaluates this use case across four dimensions — technical feasibility, migration effort, platform suitability, and risk — and recommends a migration band.
                  </div>
                </div>
                <div style={{ display: "flex", gap: 8, marginTop: 4 }}>
                  <div style={{
                    ...
                    <div style={{ fontWeight: 600, color: "var(--foreground)", marginBottom: 2 }}>Use case</div>
                    {useCase.name}
                  </div>
                  {useCase.source_platform && (
                    <div style={{
                      ...
                      <div style={{ fontWeight: 600, color: "var(--foreground)", marginBottom: 2 }}>Platform</div>
                      {useCase.source_platform}
                    </div>
                  )}
                </div>
            ...
            {isComplete && latestResult && (
              <div style={{ display: "grid", gridTemplateColumns: "340px 1fr", gap: 22 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
                  ...
                  <div style={{
                    ...
                    <Gauge
                      ...
                      size={170}
                    />
                    <PriorityBadge band={latestResult.migration_decision} solid />
                  ...
                  <div style={{ borderRadius: 14, border: "1px solid var(--border)", background: "var(--card)", padding: 18 }}>
                    ...
                    {[
                      ...
                      <div key={lbl} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "9px 0", borderBottom: "1px solid var(--border)" }}>
                        <span style={{ fontSize: 12.5, color: "var(--muted-foreground)" }}>{lbl}</span>
                        <span style={{ display: "flex", alignItems: "center", gap: 8 }}>
                          <span style={{ fontSize: 12.5, fontWeight: 500 }}>{val}</span>
                          <InputSourceBadge source={src as never} />
                        </span>
                      </div>
                    ))}
                    <div style={{ paddingTop: 10 }}>
                      <Button variant="outline" size="sm" onClick={handleBackfill} style={{ width: "100%" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 18, minHeight: 0 }}>
                  ...
                  {/* AI analysis */}
                  <div style={{
                    flex: 1, borderRadius: 14, border: "1px solid var(--border)", background: "var(--card)",
                    ...
                    <p style={{ fontSize: 13, lineHeight: 1.62, color: "var(--muted-foreground)", margin: 0 }}>
                    ...
                    {latestResult.blockers.length > 0 && (
                      <div style={{ marginTop: 16 }}>
                        <div style={{ fontSize: 11.5, fontWeight: 600, color: "var(--c-amber)", marginBottom: 8, display: "flex", alignItems: "center", gap: 6 }}>
                          <Icon name="flag" size={13} /> Blockers ({latestResult.blockers.length})
                        </div>
                        <div style={{ display: "flex", flexDirection: "column", gap: 7 }}>
                          {latestResult.blockers.map((b, i) => (
                            <div key={i} style={{
                              display: "flex", gap: 9, alignItems: "flex-start", fontSize: 12.5,
                              color: "var(--muted-foreground)", padding: "9px 12px", borderRadius: 9,
                              background: "color-mix(in oklab, var(--c-amber) 8%, transparent)",
                              border: "1px solid color-mix(in oklab, var(--c-amber) 20%, transparent)",
                            }}>
                              <span style={{ width: 5, height: 5, borderRadius: 99, background: "var(--c-amber)", marginTop: 6, flexShrink: 0 }} />
                              {b}
                            </div>
                          ))}
                        </div>
                    ...
                    <div style={{ marginTop: 16, paddingTop: 14, borderTop: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                      <div>
                        <div style={{ fontSize: 11, color: "var(--muted-foreground)" }}>Power Automate fit</div>
                        <div style={{ fontSize: 12.5, color: "var(--muted-foreground)", marginTop: 2 }}>{latestResult.power_automate_fit}</div>
                      </div>
                      <Button variant="ghost" size="sm" onClick={() => {
                        setOverrideDims({
                          technical_feasibility: latestResult.technical_feasibility,
                          migration_effort:      latestResult.migration_effort,
                          platform_suitability:  latestResult.platform_suitability,
        ...
        {viewMode === "portfolio" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <div>
                ...
                <p style={{ fontSize: 12.5, color: "var(--muted-foreground)", margin: "4px 0 0" }}>
                  Haiku scores every use case concurrently across the four dimensions.
                </p>
              </div>
              <Button size="sm" onClick={handleScoreAll} disabled={scoringAll || portfolioLoading}>
            ...
            {!portfolioLoading && (
              <div style={{ borderRadius: 13, border: "1px solid var(--border)", overflow: "hidden", background: "var(--card)" }}>
                {/* Table header */}
                <div style={{
                  display: "grid",
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        borderBottom: idx < portfolioRows.length - 1 ? "1px solid var(--border)" : "none",
                    ...
                    >
                      ...
                      {running ? (
                        <div style={{ gridColumn: "2 / 7", display: "flex", alignItems: "center", gap: 10, color: "var(--primary)", fontSize: 12 }}>
                          <span style={{
                            width: 8, height: 8, borderRadius: 99, background: "var(--primary)",
                        ...
                        <>
                          {S1_DIMS.map((d) => (
                            <div key={d.key} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
                              <span style={{ fontFamily: "var(--font-geist-mono)", fontSize: 11.5, color: "var(--muted-foreground)" }}>
                                {result[d.key] ?? 0}
                              </span>
                              <div style={{ width: 34, height: 4, borderRadius: 99, background: "var(--track)", overflow: "hidden" }}>
                                <div style={{ width: `${((result[d.key] ?? 0) / d.max) * 100}%`, height: "100%", background: d.color }} />
                              </div>
                            </div>
                          ))}
                          <div style={{ textAlign: "center", fontFamily: "var(--font-geist-mono)", fontSize: 16, fontWeight: 700, color: BAND_META[result.migration_decision]?.color }}>
                            {result.total_score}
                          </div>
                        </>
                      ) : (
                        <div style={{ gridColumn: "2 / 7", fontSize: 12, color: "var(--muted-foreground)" }}>
            ...
            {/* Follow-up callout */}
            {portfolioRows.some((r) => r.result?.follow_up_questions?.length) && (
              <div style={{
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 12.5, fontWeight: 600 }}>Follow-up questions generated for low-confidence cases</div>
      ...
      <Sheet open={overrideOpen} onOpenChange={setOverrideOpen}>
        <SheetContent className="w-[480px]">
          ...
          <p className="text-sm text-muted-foreground mt-2 mb-6">
            Adjust dimensions directly. Overrides are stored as a new run with a required reason.
          </p>
              ...
              {S1_DIMS.map((d) => (
                <div key={d.key}>
                  <div className="flex justify-between text-xs mb-1.5" style={{ color: "var(--muted-foreground)" }}>
                    <span>{d.label}</span>
                    <span className="font-mono">{overrideDims[d.key] ?? 0} / {d.max}</span>
                  </div>
                  <div className="relative h-2 rounded-full" style={{ background: "var(--track)" }}>
                    <div
                      className="h-full rounded-full transition-all"
                      style={{ width: `${((overrideDims[d.key] ?? 0) / d.max) * 100}%`, background: d.color }}
                    />
                    <input
                      type="range" min={0} max={d.max}
                      value={overrideDims[d.key] ?? 0}
                      onChange={(e) => setOverrideDims((prev) => ({ ...prev, [d.key]: +e.target.value }))}

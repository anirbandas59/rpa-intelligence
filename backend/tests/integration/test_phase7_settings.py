...
async def test_default_llm_configs_seeded(async_client, test_superuser_token, test_db_session):
    ...
    default_configs = [
        LLMConfig(stage="s1_scoring", model="claude-haiku-4-5", temperature=0.3, max_tokens=1000, is_active=True, updated_at=datetime.utcnow()),
        LLMConfig(stage="s1_followup", model="claude-haiku-4-5", temperature=0.3, max_tokens=500, is_active=True, updated_at=datetime.utcnow()),
        LLMConfig(stage="s2_extract", model="claude-haiku-4-5", temperature=0.2, max_tokens=800, is_active=True, updated_at=datetime.utcnow()),
        LLMConfig(stage="s3_narrative", model="claude-sonnet-4-5", temperature=0.5, max_tokens=1500, is_active=True, updated_at=datetime.utcnow()),
        LLMConfig(stage="s4_decompose", model="claude-sonnet-4-5", temperature=0.4, max_tokens=2000, is_active=True, updated_at=datetime.utcnow()),
    ]
...
async def test_create_prompt_variant(async_client, test_superuser_token):
    ...
    activate_response = await async_client.put(
        f"/api/v1/settings/prompts/{variant_id}/activate",
        headers=headers
    )
...
async def test_update_user_role(async_client, test_superuser_token, test_user):
    ...
    response = await async_client.patch(
        f"/api/v1/settings/users/{test_user.id}",
        json=update_payload,
        headers=headers
    )
    assert response.status_code == 200
...
async def test_deactivate_user(async_client, test_superuser_token):
    ...
    update_response = await async_client.patch(
        f"/api/v1/settings/users/{user_id}",
...
async def test_cannot_deactivate_self(async_client, test_superuser_token, test_superuser):
    ...
    response = await async_client.patch(
        f"/api/v1/settings/users/{test_superuser.id}",
        json={"is_active": False},
        headers=headers
    )
